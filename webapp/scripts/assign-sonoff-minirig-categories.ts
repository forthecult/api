...
async function main() {
  const catRes = await fetch(`${API_BASE}/api/admin/categories?limit=300`, {
    headers,
  });
  ...
  if (!validCategoryIds.has(smartHomeId)) {
    console.error(
      "Category 'smart-home' not found. Run seed-categories first (e.g. bun run db:seed-categories).",
    );
    process.exit(1);
  }
  ...
  for (const product of sonoffProducts) {
    ...
    if (!validCategoryIds.has(subId)) {
      console.warn(
        ...
        subSlug,
        "- assign smart-home only. Run seed-categories to create subcategories, then re-run.",
      );
    }
    ...
    console.log(
      ...
      product.name,
      "→",
      validCategoryIds.has(subId)
        ? `smart-home + ${subSlug} (primary)`
        : "smart-home",
    );
    updated += 1;
  ...
  const minirigRes = await fetch(
    `${API_BASE}/api/admin/products?search=Minirig&limit=20`,
    { headers },
  );
  if (!minirigRes.ok) throw new Error(`Products: ${minirigRes.status}`);
  const minirigList = (await minirigRes.json()) as {
    items?: Array<{ id: string; slug?: string; name: string }>;
  };
  const minirigProducts = (minirigList.items ?? []).filter((p) =>
    MINIRIG_SLUGS.includes((p.slug ?? "").trim()),
  );
