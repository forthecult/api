...
const UPLOADTHING_TOKEN = rawToken == null || rawToken === "" ? undefined : rawToken.trim().replace(/^['"]|['"]$/g, "");
...
if (!UPLOADTHING_TOKEN) {
  console.error("Set UPLOADTHING_TOKEN. It must be base64-encoded JSON { apiKey, appId, regions } (same as production).");
  process.exit(1);
}
...
const MINIRIG_MAP: Record<
  string,
  { slug: string; numImages: number; variantColors: string[]; mainImageAlt: string; mainImageTitle: string; imageMeta: Array<{ alt: string; title: string }> }
> = {
  "minirig-4-bluetooth-speaker": {
    ...
    imageMeta: [
      { alt: "Minirig 4 Bluetooth Speaker Black - portable wireless speaker, designed in Bristol UK", title: "Minirig 4 Bluetooth Speaker - Black" },
      { alt: "Minirig 4 Black grille and top cap", title: "Minirig 4 - Grille" },
      { alt: "Minirig 4 bottom cap and controls", title: "Minirig 4 - Bottom" },
      { alt: "Minirig 4 with protective travel case open", title: "Minirig 4 - Case open" },
      { alt: "Minirig 4 travel case closed", title: "Minirig 4 - Case closed" },
      { alt: "Minirig 4 USB-C charging cable", title: "Minirig 4 - Charger cable" },
      { alt: "Minirig 4 eco-friendly recycled packaging", title: "Minirig 4 - Packaging" },
    ],
  },
  "minirig-subwoofer-4": {
    slug: "minirig-subwoofer-4",
    numImages: 4,
    variantColors: ["Black", "Blue", "Brushed Silver", "Green", "Purple", "Red"],
    ...
    imageMeta: [
      { alt: "Minirig Subwoofer 4 Black - portable wireless subwoofer, Bristol UK", title: "Minirig Subwoofer 4 - Black" },
      { alt: "Minirig Subwoofer 4 grille top view", title: "Minirig Subwoofer 4 - Grille" },
      { alt: "Minirig Subwoofer 4 port view", title: "Minirig Subwoofer 4 - Port" },
      { alt: "Minirig Subwoofer 4 in the box", title: "Minirig Subwoofer 4 - In the box" },
    ],
  },
};
...
> = {
  "sonoff-zigbee-wireless-switch-snzb-01p": {
    ...
    imageMeta: [
      { alt: "SONOFF SNZB-01P Zigbee wireless switch", title: "SONOFF SNZB-01P - Main" },
      { alt: "SONOFF SNZB-01P wireless switch - button and mounting", title: "SONOFF SNZB-01P - Mounting" },
    ],
  },
  "sonoff-zigbee-motion-sensor-snzb-03p": {
    ...
    imageMeta: [{ alt: "SONOFF SNZB-03P Zigbee motion sensor", title: "SONOFF SNZB-03P - Main" }],
  },
  "sonoff-zigbee-door-window-sensor-snzb-04p": {
    ...
    imageMeta: [{ alt: "SONOFF SNZB-04P Zigbee door window sensor", title: "SONOFF SNZB-04P - Main" }],
  },
  "sonoff-zigbee-temperature-humidity-sensor-snzb-02p": {
    ...
    imageMeta: [{ alt: "SONOFF SNZB-02P temperature humidity sensor", title: "SONOFF SNZB-02P - Main" }],
  },
  "sonoff-l3-pro-rgbic-smart-led-strip-lights": {
    ...
    imageMeta: [{ alt: "SONOFF L3 Pro RGBIC LED strip lights", title: "SONOFF L3 Pro RGBIC - Main" }],
  },
  "sonoff-zigbee-smart-plug-iplug-s40-lite": {
    ...
    imageMeta: [{ alt: "SONOFF Zigbee smart plug S40 Lite", title: "SONOFF S40 Lite - Main" }],
  },
  "sonoff-zigbee-smart-water-valve": {
    ...
    imageMeta: [{ alt: "SONOFF Zigbee smart water valve", title: "SONOFF Smart Water Valve - Main" }],
  },
};
...
const SONOFF_FALLBACK_KEYWORDS: Record<string, string[]> = {
  ...
  "sonoff-zigbee-smart-plug-iplug-s40-lite": ["s40-lite", "iplug-s40", "s40-lite-zigbee"],
  "sonoff-l3-pro-rgbic-smart-led-strip-lights": ["l3-pro", "rgbic", "led-strip"],
};
...
async function main() {
  ...
  const listResult = (await utapi.listFiles({ limit: 300 })) as { files?: Array<{ key: string; name: string }>; hasMore?: boolean };
  const files = listResult?.files ?? [];
  if (files.length === 0) {
    console.error("No files returned from UploadThing. Check UPLOADTHING_TOKEN and app.");
    process.exit(1);
  }

  const keys = files.map((f) => f.key);
  const urlResults = await utapi.getFileUrls(keys);
  const urlList = Array.isArray(urlResults) ? urlResults : (urlResults as { data?: Array<{ key: string; url: string }> }).data ?? [];
  ...
  for (const [prefix, meta] of Object.entries(PRODUCT_MAP)) {
    ...
    payloadBySlug.set(meta.slug, {
      ...
      images,
      variantImageUrls: Object.keys(variantImageUrls).length > 0 ? variantImageUrls : undefined,
    });
  }
  ...
  for (const [slug, keywords] of Object.entries(SONOFF_FALLBACK_KEYWORDS)) {
    ...
    if (!meta) continue;
    const baseLower = (name: string) => baseNameWithoutExtension(name).toLowerCase();
    for (const f of files) {
      const url = keyToUrl.get(f.key);
      if (!url) continue;
      ...
      payloadBySlug.set(slug, {
        ...
        mainImageAlt: meta.mainImageAlt,
        mainImageTitle: meta.mainImageTitle,
        images: [{ url, alt: meta.imageMeta[0]?.alt ?? "", title: meta.imageMeta[0]?.title ?? "" }],
      });
  ...
  if (payloadBySlug.size === 0) {
    console.error("No product payloads built. UploadThing files may use different naming (e.g. minirig-4-bluetooth-speaker-0.webp).");
    process.exit(1);
  }
  ...
  for (const [slug, spec] of payloadBySlug) {
    ...
    const body = {
      ...
      images: spec.images.map((img, i) => ({ url: img.url, alt: img.alt, title: img.title, sortOrder: i })),
  ...
  console.log("Done. Updated", updated, "product(s) with images from UploadThing.");
