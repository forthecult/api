...
async function main() {
  ...
  try {
    const catalog: Array<{ id: number; name: string; type: string; variant_count: number }> = [];
    let offset = 0;
    const limit = 100;
    let hasMore = true;
    while (hasMore) {
    ...
    catalog.forEach((p) => {
      console.log(`  [${p.id}] ${p.name}  (${p.type}, ${p.variant_count} variants)`);
    });
  } catch (err) {
  ...
  try {
    const store: Array<{ id: number; name: string; synced: number; variants: number }> = [];
    let offset = 0;
    const limit = 100;
    let hasMore = true;
    while (hasMore) {
    ...
    console.error("Store products fetch failed:", err instanceof Error ? err.message : err);
  }
}
