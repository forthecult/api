...
const IMAGE_SPECS: Record<string, ImageSpec[]> = {
  "sonoff-zigbee-wireless-switch-snzb-01p": [
    {
      urls: [`${SONOFF_CDN}/SNZB-01P_Zigbee.png?v=1751008428`, `${ITEAD}/2024/01/SNZB-01P-1.jpg`],
    ...
    {
      urls: [`${SONOFF_CDN}/sonoff-zigbee-wireless-switch-snzb-01p-4.jpg?v=1751008428`, `${ITEAD}/2024/01/SNZB-01P_02.jpg`],
  ...
  "sonoff-zigbee-motion-sensor-snzb-03p": [
    {
      urls: [`${SONOFF_CDN}/SNZB-03P_Zigbee.png?v=1751008394`, `${ITEAD}/2024/01/SNZB-03P-1.jpg`],
  ...
  "sonoff-zigbee-door-window-sensor-snzb-04p": [
    {
      urls: [`${SONOFF_CDN}/SNZB-04P_3ecebce5-6f5f-4657-9d34-dc292e699965.png?v=1751008379`, `${ITEAD}/2024/01/SNZB-04P-1.jpg`],
  ...
  "sonoff-zigbee-temperature-humidity-sensor-snzb-02p": [
    {
      urls: [`${SONOFF_CDN}/SNZB-02P.png?v=1751008416`, `${ITEAD}/2024/01/SNZB-02P-1.jpg`],
  ...
  "sonoff-l3-pro-rgbic-smart-led-strip-lights": [
    {
      urls: [`${SONOFF_CDN}/L3-Pro-RGBIC.png?v=1751008522`, `${SONOFF_CDN}/L3-Pro-RGBIC.png`, `${ITEAD}/2024/06/L3-Pro-RGBIC-1.jpg`],
  ...
  "sonoff-zigbee-smart-plug-iplug-s40-lite": [
    {
      urls: [`${SONOFF_CDN}/S60ZB.jpg?v=1751264859`, `${SONOFF_CDN}/S31-Lite-zb.jpg?v=1751264594`, `${ITEAD}/2025/01/S40-Lite-Zigbee-1.jpg`],
  ...
  "sonoff-zigbee-smart-water-valve": [
    {
      urls: [`${SONOFF_CDN}/SWV.png?v=1751008511`, `${ITEAD}/2024/08/Zigbee-Smart-Water-Valve-1.jpg`],
...
function isOurCdn(url: string): boolean {
  try {
    ...
    return h.includes("utfs.io") || h.includes("ufs.sh") || h.includes("uploadthing");
  } catch {
    return false;
  }
}
...
async function fetchImageBuffer(url: string): Promise<{ buffer: ArrayBuffer; type: string } | null> {
  try {
    ...
    const type = ct.startsWith("image/") ? ct.split(";")[0]!.trim() : "image/jpeg";
    return { buffer, type };
  } catch {
    return null;
  }
...
async function uploadToCdn(
  ...
  const file = new File([buffer], filename.endsWith(`.${ext}`) ? filename : `${filename}.${ext}`, {
    type: mimeType,
  });
...
async function main() {
  const listRes = await fetch(`${API_BASE}/api/admin/products?search=Sonoff&limit=20`, {
    headers: { Authorization: `Bearer ${API_KEY}` },
  });
  ...
  for (const product of products) {
    ...
    for (let i = 0; i < specs.length; i++) {
      ...
      if (!data && !newImages.some((img) => img.alt === spec.alt)) {
        console.warn(`  Could not fetch image for ${product.name}: ${spec.title}`);
        continue;
      }
      if (data) {
        const name = slug.replace(/^sonoff-/, "") + (i > 0 ? `-${i}` : "");
        const cdnUrl = await uploadToCdn(data.buffer, data.type, `sonoff-${name}`);
        if (cdnUrl) {
          newImages.push({ url: cdnUrl, alt: spec.alt, title: spec.title });
          console.log("  Uploaded:", spec.title, "→", cdnUrl.slice(0, 50) + "...");
        }
      }
    }
    ...
    updatedCount += 1;
    console.log("Updated", product.name, "with", newImages.length, "image(s) from our CDN.");
    await new Promise((r) => setTimeout(r, 600));
  }

  if (updatedCount > 0) {
    console.log("Done. Sonoff images are now on our CDN (" + updatedCount + " product(s) updated).");
  } else {
    console.error(
