...
async function fetchImageBuffer(url: string): Promise<{ buffer: ArrayBuffer; type: string } | null> {
  try {
    ...
    const type = ct.startsWith("image/") ? ct.split(";")[0]!.trim() : "image/webp";
    return { buffer, type };
  } catch {
    return null;
  }
...
async function main() {
  ...
  if (!listRes.ok) throw new Error(`Products: ${listRes.status} ${await listRes.text()}`);
  ...
  for (const product of products) {
    ...
    const productLabel = slug.includes("subwoofer") ? "Subwoofer 4" : "4 Bluetooth Speaker";
    const variants = existingVariants.map((v) => {
      const color = (v.color ?? "").trim();
      const sourceUrl = color ? spec.variantImageByColor[color] : null;
      const imageUrl = sourceUrl ? urlToCdn.get(sourceUrl) ?? sourceUrl : undefined;
      return {
        id: v.id,
        color: v.color ?? null,
    ...
    const body = {
      ...
      images: newImages.map((img, i) => ({ url: img.url, alt: img.alt, title: img.title, sortOrder: i })),
      variants,
    };
    ...
    updatedCount += 1;
    console.log("Updated", product.name, "with", newImages.length, "image(s) + variant images from our CDN.");
    await new Promise((r) => setTimeout(r, 600));
  }

  if (updatedCount > 0) {
    console.log("Done. Minirig images are now on our CDN (" + updatedCount + " product(s) updated).");
  } else {
    console.error(
