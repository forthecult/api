...
- **Seed images upload but show as blank/grey placeholder**  
  Some vendor CDNs (e.g. **Seeed Studio** `media-cdn.seeedstudio.com`) return a small placeholder image instead of the real product photo. The upload script now **skips** images whose source response is under 12 KB (or under 35 KB for Seeed, to skip the gray second placeholder) so we don’t upload placeholders to UploadThing. In the seed log you’ll see “Source image too small” and “Skipped (fetch failed or placeholder)” for those URLs. **Fix:** Re-run the upload script so skipped placeholder URLs are removed from the DB. For products left with no image, update the seed data to use real image URLs (e.g. from the product page or another host). Pacsafe and other Shopify CDN URLs return full-size images and upload correctly.
