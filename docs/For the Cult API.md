...
**The most AI-agent-friendly eCommerce API.** Designed for seamless integration with AI assistants, chatbots, and autonomous agents. Browse products, place orders, and pay — all through simple API calls. The store offers lifestyle, wellness, and longevity products (apparel, accessories, eSIMs, membership) from **For the Cult** Store.
...
**Security & PII:** Order details (email, shipping address) are only returned when the caller is the order owner (session or confirmation token `ct`) or an admin. The status endpoint returns only `orderId`, `status`, and `paidAt` — no PII. Public documentation and examples use placeholder data only.
...
**GET /api/orders/{orderId}** — optional: **?ct= confirmationToken**

Full order details including items, shipping address, payment summary, and timeline.

**Access:** (1) Authenticated session (owner) or admin, or (2) valid `ct` (confirmation token from order confirmation email) for orders created &lt;1 hour ago. Without auth or valid `ct`, PII (email, full shipping address) is redacted. For status-only checks use **GET /api/orders/{orderId}/status**.
...
| **Orders** | | |
| GET | /api/orders/{orderId}/status | Lightweight status for polling (no auth) |
| GET | /api/orders/{orderId} | Full order details (auth or ?ct= token; PII redacted otherwise) |
| POST | /api/orders/{orderId}/cancel | Cancel pending order (auth or body.lookupValue to prove ownership) |
