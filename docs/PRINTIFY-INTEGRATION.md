...
**Production vs staging:** Printify has one store per shop; the webhook URL you register is where **all** events for that shop are sent. To have products land in **production**, register webhooks from the **production** app (so `NEXT_PUBLIC_APP_URL` in production is your production domain). If you previously registered from staging, Printify will send events to staging until you register again from production (you can delete old webhooks first via `POST /api/admin/printify/webhooks` with `{ "action": "delete_all" }` from staging, then register from production). Use the **production** Printify API token and shop ID in production env so the same shop receives the new production URL.
