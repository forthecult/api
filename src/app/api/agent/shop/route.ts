...
import { runProductSearch } from "~/lib/product-search";
import {
  checkRateLimit,
  getClientIp,
  getRateLimitHeaders,
...
type SourceFilter = "all" | "marketplace" | "store";
...
export async function POST(request: NextRequest) {
  ...
  try {
    const body = (await request.json().catch(() => ({}))) as ShopRequest & {
      source?: string;
    };
    ...
    const halfLimit =
      wantMarketplace && wantStore ? Math.ceil(limit / 2) : limit;
    ...
    const marketplaceProducts = (marketplaceResult?.products ?? []).map(
    ...
    const parsed = { category, priceMax, priceMin, query: query || message };
...
function generateAiReply(
  message: string,
  products: ShopProduct[],
  parsed: { priceMax?: number; priceMin?: number; query: string },
): string {
  ...
  const marketplaceCount = products.filter(
    (p) => p.source === "marketplace",
  ).length;
...
function mapStoreProduct(
  p: ProductSearchResultItem & { badge?: string; source?: string },
): ShopProduct {
  return {
    badge: p.badge,
    currency: "USD",
