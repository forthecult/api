...
export function bondingBuyQuote(
  ...
  return tokenOut > state.realTokenReserves ? state.realTokenReserves : tokenOut;
}
...
export async function estimateSolFromCultBonding(
  ...
  return { solLamports: Number(minSolOut), solAmount };
}
...
export async function buildBondingBuyInstructions(
  ...
  solLamports: number,
  slippagePercent: number = 1.5,
): Promise<{ instructions: TransactionInstruction[]; estimatedCultRaw: string }> {
  const mint = new PublicKey(getCultSwapMint());
  const state = await getBondingCurveState(connection, mint);
  ...
  const buyIx = new TransactionInstruction({
    ...
    keys: [
      { pubkey: accs.global, isSigner: false, isWritable: false },
      { pubkey: FEE_RECIPIENT, isSigner: false, isWritable: true },
      { pubkey: mint, isSigner: false, isWritable: false },
      { pubkey: accs.bondingCurve, isSigner: false, isWritable: true },
      { pubkey: accs.associatedBondingCurve, isSigner: false, isWritable: true },
      { pubkey: accs.userAta, isSigner: false, isWritable: true },
      { pubkey: userPublicKey, isSigner: true, isWritable: true },
      { pubkey: SystemProgram.programId, isSigner: false, isWritable: false },
      { pubkey: CULT_TOKEN_PROGRAM, isSigner: false, isWritable: false },
      { pubkey: accs.creatorVault, isSigner: false, isWritable: true },
      { pubkey: accs.eventAuthority, isSigner: false, isWritable: false },
      { pubkey: PUMP_PROGRAM_ID, isSigner: false, isWritable: false },
      { pubkey: accs.globalVolumeAccumulator, isSigner: false, isWritable: true },
      { pubkey: accs.userVolumeAccumulator, isSigner: false, isWritable: true },
      { pubkey: accs.feeConfig, isSigner: false, isWritable: false },
...
export async function buildBondingSellInstructions(
  ...
  cultRaw: string,
  slippagePercent: number = 1.5,
): Promise<{ instructions: TransactionInstruction[]; estimatedSolLamports: number }> {
  const mint = new PublicKey(getCultSwapMint());
  const state = await getBondingCurveState(connection, mint);
  if (!state || state.complete) {
    throw new Error("Pool account not found");
