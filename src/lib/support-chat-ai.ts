...
function extractTextFromOpenResponsesOutput(output: unknown): null | string {
  ...
  for (const item of output) {
    if (
      ...
      const content = (item as { content?: { text?: string; type?: string }[] })
        .content;
  ...
  try {
    const body: Record<string, unknown> = {
      ...
      messages: [
        ...
        { content: truncated || "Hello", role: "user" },
      ],
...
async function generateViaOpenClaw(
  ...
  const body: Record<string, unknown> = {
    max_tokens: 500,
    messages: [{ content: truncated || "Hello", role: "user" }],
